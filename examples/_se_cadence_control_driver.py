...
NET = {14: pn.case14, 30: pn.case30, 57: pn.case57, 118: pn.case118, 300: pn.case300}[CASE]
...
CAD, ANCHOR_DT = float(os.environ.get("SE_CAD", "2.0")), 300.0   # CONTROLLED VARIABLE: SE step cadence (s)
...
# CONTROL INVARIANT: keep the SAME 256-step sequence length at every cadence (so the GRU/EKF see identical-length
# sequences and the WLS/EKF/learned comparison is apples-to-apples), and scale the number of real 5-min NYISO
# anchors so they always fully COVER the L*CAD wall-clock window (no spline extrapolation). At CAD=2 this yields
# NA=6 -> byte-identical to the original 2 s run; at CAD=60 it grows to cover the longer window. The ONLY physical
# thing that changes with cadence is the per-step spacing, hence the OU decorrelation (PHI below) and the per-step
# state move -- i.e. the temporal prior weakens, which is exactly the control we want to isolate.
NA = max(6, int(np.ceil((L - 1) * CAD / ANCHOR_DT)) + 2)                          # anchors cover the L*CAD window
...
def make_sequence(rs, attack):
    ...
    if attack:                                                                   # stealthy angle ramp on 1-3 buses
        ...
        # ramp lasts a fixed PHYSICAL 30-120 s regardless of cadence; convert to steps, guarding dur>=1 (at 60 s
        # cadence a 30-120 s ramp spans <=2 samples -- an honest consequence of coarse sampling, not a bug).
        a0 = int(rs.integers(int(L * 0.3), int(L * 0.6)))
        _lo = max(1, int(round(30 / CAD))); _hi = max(_lo + 1, int(round(120 / CAD))); dur = int(rs.integers(_lo, _hi))
...
res = dict(system=f"ieee{CASE}", model="AC-SE (EKF + learned residual)", cadence_s=CAD, seeds=list(SEEDS),
           ...
           n_anchors=NA, seq_len=L, phi_ou=round(float(np.exp(-CAD / TAU)), 5))
outp = os.path.join(RES, f"se_cadence_ctrl_ieee{CASE}_cad{int(round(CAD))}.json")
