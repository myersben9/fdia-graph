...
# 1) LOAD — one call. Auto-downloads the newest release and caches under ~/.cache/fdia_graph.
...
# 2) TRAIN-READY — a PyTorch DataLoader with the measurement graph + per-bus labels per batch.
...
# 4) ANY FRAMEWORK — pull the whole split as numpy / torch / pandas (tensorflow via .to_tf()).
