...
| an argument with a fixed set of values is a `Choice` | declared once, as an enum beside the code it belongs to (`choices.Choice`), and converted where it enters (`Units(units)`), which raises the one shared message; no hand-written "must be one of" check, and a table keyed by the choice is keyed by its members | `src/fdia_graph/choices.py` |
