...
| 4 | data | the eight systems regenerated with the seven families (`examples/_build_timelines_v080.py`), pools converted, **one file per system**, one release `v0.8.0`; the registry maps each release tag to its layout and sha256s, so `release="v0.7.2"` still loads the old shards; `tools/upload_assets.py` uploads | yes |
