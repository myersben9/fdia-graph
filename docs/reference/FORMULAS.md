...
| local false state | `formulas.network.local_ac_solve`, `FdiaGenerator.solve_local` | S_i(V) = V_i conj(Σ_j Y_ij V_j) = S_target_i on the interior, every other V held true; Newton on [θ_I, \|V\|_I] | [WU26], [AE04, ch. 2] | Aq, At, Al, Am |
...
| tamper set of a stealthy frame | `engine.records._changed_meters`, called by `_stealthy_frame` | the metered channels whose noiseless value the local false state moves: the interior's and boundary's injections and voltages, the flows on branches touching the interior | [WU26] | Aq, At, Al, Am |
