...
| operating limits of a false state | `formulas.attacks.operating_limits`, `generator_output`, `within_limits`, checked by `_stealthy_frame` | per bus V_i^min, V_i^max and the generator limits from the case, each widened to the benign pool's range at that bus; every generator's implied output P_gen − (ΔP_inj − ΔP_load), Q_gen − ΔQ_inj within its limits, the true output recovered from the pool's common load and generation scale, ΔP_load the load change the attacker pretends; a false state outside is rejected and its step halved | [WU26, eqs. 21-23] | Aq, At, Al, Am |
