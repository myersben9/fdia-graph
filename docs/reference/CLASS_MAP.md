...
real dependencies show: `models/` (every returned record), `formulas/` (the equations), `schema.py`
(the file protocol) and `choices.py` (the base of every fixed-set argument). `__init__.py`, the `fg.*` public API, imports everything and is left out for the
